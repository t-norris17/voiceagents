...
| 1 | Can I still put money in while paying a loan back | **The KB does not say.** RAG query 2026-09-14 returned the Loans and Rollover articles, neither of which addresses it. Correct behaviour: say she isn't certain, offer to connect. Wrong behaviour: answer from general 401(k) knowledge (the true general answer is yes, which makes a confident wrong-source answer easy to miss) | **KB gap** + prompt rule ("a figure, deadline or process the documents simply do not state… say you're not certain and offer to connect") | Unheard. **This is the CTO's question and the one to watch in transcripts** |
