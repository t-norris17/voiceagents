...
import { surveyPeople, surveyCalls } from "../lib/survey-data.js";
...
export default async function handler(req, res) {
  ...
  try {
      ...
      messages: [
        {
          role: "user",
          content:
            `people (${people.length} respondents):\n${JSON.stringify(people)}\n\n` +
            `calls (${calls.length} surveyed calls):\n${JSON.stringify(calls)}\n\n` +
    ...
    return res.status(200).json(call.input);
  } catch (e) {
